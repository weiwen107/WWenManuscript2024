...
%frequency quantification only? (events with multiple peaks will be
%excluded if frq_only == 1)
frq_only = 0;
...
if save_results == 1
    if risetime_cutoff < 2.01 %10-90, 2ms (regular setting for mEPSC)
        ...
        if frq_only == 1
            cd(strcat(fp_analyzed_data,'/rise_',num2str(1),'/frq_only'))
        end
    else
        cd(strcat(fp_analyzed_data,'/rise'))

        if frq_only ==1
            cd(strcat(fp_analyzed_data,'/rise','/frq_only'))
        end
